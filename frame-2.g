		...
		if i = 1 then
			...
			if i mod 2 = 0 then
				if j mod 2 = 0 then
					...
					coordinates_assembly:=Concatenation(coordinates_assembly,blcoords+[2+a,0,0]*(i-1)+[0,2+a,0]*(j-1));
				fi;
			else
				if j mod 2 = 0 then
					coordinates_assembly:=Concatenation(coordinates_assembly,trcoords+[2+a,0,0]*(i-1)+[0,2+a,0]*(j-1));
				else
					coordinates_assembly:=Concatenation(coordinates_assembly,tlcoords+[2+a,0,0]*(i-1)+[0,2+a,0]*(j-1));
				fi;
			fi;
...
ComponentsOuterHull([s_assembly, coordinates_assembly*1.0], "frame-2", 0.0001, false);
