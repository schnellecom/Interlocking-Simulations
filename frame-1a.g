...
for i in [1..x] do
	for j in [1..y] do
		if i = 1 then
			...
			coordinates_assembly:=Concatenation(coordinates_assembly,coordinates+[2+a,0,0]*(i-1)+[0,2+a,0]*(j-1));
...
ComponentsOuterHull([s_assembly, coordinates_assembly*1.0], "frame-1a", 0.0001, false);
